...
- Current implementation version: `3.1.4`.
...
- Every `sipx` module carries an RFC-referencing module docstring (purpose + explanation + `References:` block), added `3.1.2`. `sipx.protocol` is the `AsyncClient` runtime layer; `sipx.rfc` is standalone, test-only extension handlers (PRACK/events/presence/outbound/DNS) not yet wired into `AsyncClient`. `sipx.sip` is the parallel sans-I/O toolkit the `AsyncClient` does not import (root re-exports `Dialog`/transactions/`create_*_request` from `sip/`, not from `protocol/`).
- Known `AsyncClient` RFC gaps (documented in README §"AsyncClient status and RFC limitations", `3.1.4`): no RFC 3261 §17 timer firing / UDP retransmission; no auto-ACK for non-2xx INVITE finals and no CANCEL; response correlation is `Call-ID`+CSeq-number only (no Via branch/source check); Digest is MD5-only (no RFC 8760 SHA-256); inbound requests are not auto-routed (call `handle_request` yourself); `Content-Length` is not added to every request.
- Repo is a `uv` workspace: root `sipx` is SIP-only; apps under `apps/*` (`harness`, `cli`, `asterisk`, `llm`, `scenarios`, `stt`, `tts`) import root as workspace dep.
