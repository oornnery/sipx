...
| O16 | Reorg of duplicated SIP stacks (awaits user OK)? | unify on `protocol/` + remove `sip/`/`rfc/` orphans + rename `rfc/` + align public API; OR keep `sip/` as public API and make `AsyncClient` use it; OR only rename `rfc/` now | Phase 2 of the core review; breaking change, deferred until user picks direction. |
| O17 | Security/RFC hardening roadmap (awaits user OK)? | P0: response correlation by branch+source, CRLF sanitization, `Content-Length` everywhere, TCP/TLS reassembly cap; P1: §17 timers/retransmission, auto-ACK for INVITE error, CANCEL, real Via sent-by + `rport`; P2: PRACK/100rel in client, Digest SHA-256 + incremental `nc` + multi-challenge | Phase 3 of the core review; prioritized but not started pending user go-ahead. |
