...
Project planning environment was initialized from `IDEA.md`. Blocks `0.2.0` through `1.8.0` added initial product code and then split it into a `uv` workspace: root `sipx` is core-only; app packages under `apps/*` own CLI, softphone, Asterisk, LLM, scenarios, STT, and TTS surfaces. The codebase includes harness core, mock backend, scenario artifacts, reports, profiles, mixed actor binding, media primitives, redaction, SIP parser primitives, SDP audio offer/answer, RTP packet stats, RFC4733 DTMF, SIP dialog skeletons, INVITE/non-INVITE client transactions, REGISTER helper/flow, Digest auth helper, UAS INVITE skeleton, BYE helper, real UDP Native SIP transport/backend, strict INVITE/ACK/BYE call flow, CANCEL runtime, REGISTER over-UDP orchestration, INVITE and BYE Digest retry, transaction retransmission timers, parser fuzz tests, app-packaged Asterisk ARI backend/Stasis helpers, app-packaged headless native softphone, app-packaged CLI, app-packaged LLM client/examples, Docker Asterisk lab, lab-only native SIP hooks, GitHub CI/release workflows, fail-fast phone CLI config validation, curl-like SIP request commands, redacted packet debug output, native softphone SDP negotiation, in-dialog SIP INFO DTMF, runnable examples, and passing `uv run ty check` validation. SPEC T1-T47 plus V24-V41/B4-B12 are complete.
...
- `uv run --package sipx-cli sipx --help`: pass; commands include `options`, `message`, `request`, `profile`, `phone`, `register`, `unregister`, `call`, and `listen`.
- `uv run --package sipx-cli sipx request --help`: pass; help includes `--username`, `--password`, and `--debug-sip` flags.
- `uv run --package sipx-cli sipx call --help`: pass; help includes `--debug-sip`, media host/port, and codec flags.
- `python -m pytest`: pass, 129 passed, 3 skipped after `1.8.0` workspace split.
...
- Real proxy call: INVITE Digest challenge retried; confirmed call then challenged BYE and authenticated BYE retry reached `200 OK`; no real secret/account/proxy values are persisted in repo files.
- `python -m pytest tests/test_recorder_reports_profiles.py tests/test_protocol_fuzz.py apps/cli/tests/test_cli.py tests/test_harness_scenario.py apps/asterisk/tests/test_asterisk_integration.py`: pass, guarded Asterisk integration skipped unless enabled.
...
- `uv build --all-packages --out-dir /tmp/opencode/sipx-build-1.8.0-final-docs`: pass, built root and app package sdists/wheels outside the repo.
- `python -m ty check`: blocked, active interpreter has no `ty` module.
- `uv run ty check`: pass after type-hardening fixes.
- Runnable examples: pass for direct LLM semantic smoke, native CLI flow printer, call-with-DTMF help, Mizu register help, and `uv run --package sipx-cli sipx call --help`.
