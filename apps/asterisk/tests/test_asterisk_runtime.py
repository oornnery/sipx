...
from sipx_harness import RuntimeCapability, Timeline
...
def test_asterisk_runtime_records_ari_websocket_events_to_timeline() -> None:
...
async def _record_ari_websocket_events_to_timeline() -> None:
    ...
    runtime = AsteriskRuntime(timeline=timeline, actor_id="pbx")

    events = [event async for event in runtime.events(source())]

    assert runtime.supports(RuntimeCapability.ASTERISK_ARI)
...
def test_asterisk_runtime_maps_control_methods_to_timeline() -> None:
...
async def _map_control_methods_to_timeline() -> None:
    ...
    runtime = AsteriskRuntime(client=client, timeline=timeline, actor_id="pbx")

    channel = await runtime.originate(
        "PJSIP/alice",
        app_args="scenario-1",
        variables={"TRACE_ID": "run-1"},
        channel_id="chan-1",
    )
    await runtime.answer_channel(channel.id)
    await runtime.send_dtmf(channel.id, "12#")
    playback = await runtime.play_channel(
        channel.id, "sound:demo", playback_id="play-1"
    )
    bridge = await runtime.create_bridge(bridge_id="bridge-1")
    await runtime.add_channel_to_bridge(bridge.id, channel.id)
    await runtime.remove_channel_from_bridge(bridge.id, channel.id)
    await runtime.hangup_channel(channel.id, reason="normal")
...
def test_asterisk_runtime_maps_known_ari_events_to_timeline() -> None:
...
async def _select_websocket_media_mvp() -> None:
    ...
    channel = await runtime.create_websocket_media_channel(
        "WebSocket/sipx",
        media_config=config,
        channel_id="media-1",
    )

    assert runtime.supports(RuntimeCapability.MEDIA)
    ...
    with pytest.raises(AsteriskAriError, match="MVP path"):
        await runtime.create_media_port(
...
async def _send_and_receive_media_frames() -> None:
    ...
    runtime = AsteriskRuntime(timeline=timeline, actor_id="pbx")
    port = await runtime.create_media_port(
