...
print(f"Silence: {len(pcm)} bytes ({len(pcm) / 16000:.1f}s)")

# White noise
noise = NoiseGenerator(amplitude=0.3)
pcm = noise.generate(duration_ms=100)
print(f"Noise: {len(pcm)} bytes, non-zero: {any(b != 0 for b in pcm)}")
...
for digit in "0123456789*#ABCD":
    ...
    print(f"  DTMF '{digit}': {freqs[0]}+{freqs[1]}Hz = {len(pcm)} bytes")
