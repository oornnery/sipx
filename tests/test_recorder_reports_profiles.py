...
def test_mixed_scenario_binds_native_asterisk_and_mock_actors() -> None:
    ...
    mixed = MixedScenario(
        MixedActorSpec("caller", backend="native", kind="softphone"),
        MixedActorSpec("pbx", backend="asterisk", kind="asterisk"),
        MixedActorSpec("agent", backend="mock", kind="remote"),
    )

    actors = mixed.bind(harness)

    assert actors["caller"].backend_name == "native"
    assert actors["pbx"].kind == "asterisk"
    assert actors["agent"].backend_name == "mock"
