...
async def test_parse_uri_with_transport() -> None:
    ...
    resolver = SipDnsResolver()
    scheme, host, port, transport = resolver._parse_uri('sip:bob@example.com;transport=tcp')
    assert scheme == 'sip'
    assert host == 'example.com'
    assert port is None
...
async def test_parse_uri_with_port_and_transport() -> None:
    ...
    resolver = SipDnsResolver()
    scheme, host, port, transport = resolver._parse_uri('sip:bob@example.com:5061;transport=tls')
    assert scheme == 'sip'
    assert host == 'example.com'
    assert port == 5061
...
async def test_resolve_with_user_part() -> None:
    ...
    resolver = SipDnsResolver()
    resolver._mock_records = {
        'example.com': [('example.com', 5060, 'udp')]
    }
